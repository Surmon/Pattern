...
public final class Vec3 extends AbstractVec {
    ...
    public static final int SIZE = 3 * Float.SIZE;
    ...
    public Vec3(FloatBuffer buffer) {
        ...
        assert buffer.capacity() >= 3 : "Invalid vector buffer length";
    ...
    /**
     * Computes euclidean distance between this vector and given vector.
     * @param vec
     * @return 
     */
    public float distance(final Vec3 vec) {

        float dx = this.x - vec.x;
        float dy = this.y - vec.y;
        float dz = this.z - vec.z;

        return FastMath.sqrtFast(dx * dx + dy * dy + dz * dz);
    }
